...
foreach ( $CONFIG[ 'wikis' ][ 'allwikis' ] as $sWiki => $aWiki ) {
	...
	$bHasPages = true;
	$sContinue = '&continue=';
	$sPages = file_get_contents_gzip( $aWiki[ 'api' ] . '?action=query&format=json&list=recentchanges&rcprop=title&rcnamespace=0|4|10&rclimit=' . $CONFIG[ 'listlimit' ] . $sContinue );
